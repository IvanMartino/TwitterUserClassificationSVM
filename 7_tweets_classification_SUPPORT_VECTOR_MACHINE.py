...
def main():
    ...
    m=8000 #number of elements in the traning set
            #
    ...
    print("-----------------------------")
    print("METHOD - MULTI-CLASS SUPPORT VECTOR MACHINE")
    print("-----------------------------")


    print("Hello,\nwe will use a Support Vector Machine algorithm \nto classify twitter users\n")
    setpath()
    ...
    model_SVM.fit(X_train[0:m], Y_train[0:m])

    #clf.predict(X_dev)
    acc=model_SVM.score(X_train[0:m], Y_train[0:m])
    acc_dev=model_SVM.score(X_dev, Y_dev)
    print("Accuracy on the training set", acc)
    print("Accuracy on the development set", acc_dev)


    df_score, df_fp, df_pre=compute_accuracies(model_SVM,1,screen_names, X_train[:,0:n_features], X_dev[:,0:n_features], Y_train, Y_dev)
